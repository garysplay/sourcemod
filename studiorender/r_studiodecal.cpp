		...
		if (!pMorph)
		{
			...
			else
			{
				...
				else if (m_VertexCache.IsVertexFlexed(n))
				{
					...
					R_StudioTransform(pFlexedVertex->m_Position.AsVector3D(), pBoneWeights, pCachedVert->m_Position.AsVector3D());
					R_StudioRotate(pFlexedVertex->m_Normal.AsVector3D(), pBoneWeights, pCachedVert->m_Normal.AsVector3D());
				}
				else
				{
					Assert(pMesh);
