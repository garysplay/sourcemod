	...
	for (j = 0; j < pMeshData->m_NumGroup; ++j)
	{
		...
		for (int i = 0; i < pGroup->m_NumVertices; ++i)
		{
			...
			if (m_VertexCache.IsVertexFlexed(n))
			{
				...
				R_StudioTransform(pFlexVert->m_Position.AsVector3D(), &vert.m_BoneWeights, pWorldVert->m_Position.AsVector3D());
				R_StudioRotate(pFlexVert->m_Normal.AsVector3D(), &vert.m_BoneWeights, pWorldVert->m_Normal.AsVector3D());
