...
{
public:
	CDispMeshEvent( int *pIndices, int indexCount, CCoreDispInfo *pDispInfo );
...
CDispMeshEvent::CDispMeshEvent( int*pIndices, int indexCount, CCoreDispInfo *pDispInfo )
{
	m_pIndices = pIndices;
...
void Disp_BuildVirtualMesh( int contentsMask )
{
	...
	g_PhysDispSize = totalSize + sizeof(dphysdisp_t) + (sizeof(int) * header.numDisplacements);
