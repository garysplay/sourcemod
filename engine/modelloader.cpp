...
void Mod_LoadVertNormalIndices( void )
{
    ...
    // get a pointer to the vertex normal data.
	int*pIndices = ( int* )lh.LumpBase();

	int count = lh.LumpSize() / sizeof(*pIndices);
	int*out = (int *)Hunk_AllocName( lh.LumpSize(), va( "%s [%s]", lh.GetLoadName(), "vertnormalindices" ) );
...
void Mod_LoadPrimIndices( void )
{
	...
	count = lh.LumpSize() / sizeof(*in);
	out = (int*)Hunk_AllocName( count*sizeof(*out), va("%s [%s]", lh.GetLoadName(), "primindices" ) );
...
void Mod_LoadLeafMinDistToWater( void )
{
	...
	else
	{
		...
		count = lh.LumpSize() / sizeof(*in);
		out = (int*)Hunk_AllocName( count*sizeof(*out), va( "%s [%s]", lh.GetLoadName(), "leafmindisttowater" ) );
...
{	
	...
	if (count < 1 || count >= MAX_MAP_SURFEDGES)
		...
		lh.GetMapName(), count);
	out = (int*)Hunk_AllocName( count*sizeof(*out), va( "%s [%s]", lh.GetLoadName(), "surfedges" ) );
